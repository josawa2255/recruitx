# イチオシ採用（リポジトリ名 recruitx） — Claude Code 作業ガイド

> イチオシ採用 採用LP の入口ドキュメント（2026-07-07にリクルートXから改名。リポジトリ名・内部識別子は `recruitx` のまま維持）。**まずここを読み、変更時はどのドキュメントを更新するか [docs/DOC-RULES.md](docs/DOC-RULES.md) で判断する。**
...
| 項目 | 値 |
|------|-----|
| サービス | イチオシ採用（表示名。旧名リクルートX／英語表記は使わない） |
| 運営会社 | コンテンツエックス株式会社 |
| 社内プレフィックス | `[R]` = イチオシ採用（兄弟: `[C]`=Contents X / `[B]`=BizManga）。GitHubリポジトリ名・クラスプレフィックス等の内部識別子は引き続き `recruitx`/`rx-` |
| ドメイン | `ichioshi.contentsx.jp`（GitHub Pages + CNAME / DNS お名前.com） |
| クラスプレフィックス | `.rx-` / CSS変数 `--rx-*` / localStorage `rx-lang`（表示名変更後も維持。変更しない） |

## WordPress連携（cms.contentsx.jp・⭐スキル必須）

イチオシ採用のコンテンツをWPで管理する作業（CPT追加・メニュー・CORS・API）は、**ユーザーレベルスキル `wp-service-onboard` を必ず発動**して進める（「WPに引き込んで」「CPT追加」等で自動発動）。
...
- 本文の正本はWP（採用事例=rx_case 専用CPT / コラム=共有 `cx_column` に相乗り＝掲載先「リクルートX」にチェック※WP管理画面の項目名は未改名。ビズマンガ/ContentsX と同じ「コラム」メニュー）。**事例は3,000〜5,000字・コラムは7,000字以上**。コラム一覧フロントは `column.html`＋`tools/build-columns.py`（`/columns?site=recruitx` を取得→静的生成）
