...
## 実装済み：関連事例の内部リンク（2026-06-14）

詳細ページの本文とCTAの間に「関連する採用成功事例」セクション（`.rx-rel`）を追加。事例クラスタ内の回遊・内部リンク強化が目的。

- **選定**: `build_related()`（build-cases.py）が同じ業態タグ（`tag_slugs`）の共有数でスコアリングし**上位6件**を自動抽出（同点はAPI新着順）。共有が無い社は同カテゴリ→全体でフォールバックし常に6件表示。例: ganesha⇄shogun-burger（ハンバーガー）、arc-craft→sweep（清掃）が自然に相互リンク。
- **表示**: **角丸なしの四角いサムネカードを横スクロール**（最大6件・scroll-snap）。1枚＝アイキャッチ＋業態＋会社名＋一文＋「詳しく見る」。詳細ページ間リンクは相対パス `../{slug}/`。
- **マークアップ/CSS**: テンプレに `{{related_cases}}`。`.rx-rel*` は case.css を読まない詳細ページ用に **case-detail.css に自己完結で定義**（rx-ccardとは別系統。SVGはサイズ固定で肥大化防止）。横スクロール（flex+overflow-x、PC約3枚見え／モバイル幅78%）。
