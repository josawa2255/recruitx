...
- **選定**: `build_related()`（build-cases.py）が同じ業態タグ（`tag_slugs`）の共有数でスコアリングし上位3件を自動抽出（同点はAPI新着順）。共有が無い社は同カテゴリ→全体でフォールバックし常に3件表示。例: ganesha⇄shogun-burger（ハンバーガー）、arc-craft→sweep（清掃）が自然に相互リンク。
- **表示**: サムネカード3件（アイキャッチ＋業態＋会社名＋一文＋「詳しく見る」）。詳細ページ間リンクは相対パス `../{slug}/`。
- **マークアップ/CSS**: テンプレに `{{related_cases}}`。`.rx-rel*` は case.css を読まない詳細ページ用に **case-detail.css に自己完結で定義**（rx-ccardとは別系統。SVGはサイズ固定で肥大化防止）。PC3列／768px以下1列。
