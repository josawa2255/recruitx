...
| 導入事例 | `case.html` | `js/main.js` + `js/case.js`（業種フィルタ） | 実装済み（カード写真11枚は生成待ち・プレースホルダー表示中） |
...
- 導入事例は別ページ `case.html`（ナビ「事例」から遷移。`#case` アンカーは廃止）
  ...
  - カード写真は `images/placeholders/photo.svg` 表示中。生成後は `images/case/` に配置して差し替え（`img` の `data-photo` 属性が予定ファイル名）
  - このページのみナビをモック準拠に上書き（`.rx-casepage` スコープ）。「料金プラン」「お役立ち資料」はページ未作成のためプレースホルダーリンク
