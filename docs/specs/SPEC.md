...
| 導入事例 | `case.html` | `js/main.js` + `js/case.js`（業種フィルタ/サマリー数字カウントアップ） | 実装済み |
...
- 導入事例は別ページ `case.html`（ナビ「事例」から遷移。`#case` アンカーは廃止）
  - 構成: ページヘッダー（サマリー数字3つ）→ ピックアップ事例（Before→After 大型パネル）→ 事例一覧（業種フィルタ: すべて/飲食/清掃・家事代行、カード10枚）→ CTA（contact.html へ）
